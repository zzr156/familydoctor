...
<script type="text/javascript">
	function uiTmpToSample(id,data,idx){
		var t = $($("#" + id + "").html());
...
<script type="text/javascript">
	...
	function findTable(){
		...
		doAjaxPost('<%=request.getContextPath()%>/flow.action?act=stateListCmm',{qvoJson:JSON.stringify(qvo),state:"3"},callDataToSample);
	}
	...
	function forReceiveSample(ui){
    	idd=$(ui).data("vo").id;
   		 doAjaxPost('<%=request.getContextPath()%>/aids.action?act=isReceiveSampleCmm',{id:$(ui).data("vo").id},callDataToForSample);
    }
    function callDataToForSample(data){
    	//alert(JSON.stringify(data));
    	if(data.msg == 'true'){
    		var url = '<%=request.getContextPath()%>/aids.action?act=forLook&handle=sample&isBack=hide&id='+idd;
			var code = idd+"12";
			var title = "接样";
			comTabs(url,code,title);
		}else{
