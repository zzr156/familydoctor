...
<body class="gray-bg">
	                ...
	                <div class="ibox-content" id="form_qvo">
	                    <form method="get" class="form-horizontal">
	                        ...
	                        <div class="text-center" style="border-top: 1px dashed #D7D7D7;padding-top: 10px;">
	                        	<a class="btn btn-primary system position_list" onclick="findTable();">查询</a>
	                        </div>
	        ...
	        <div class="col-sm-12">
	            <div class="ibox float-e-margins">
	            	<div class="ibox-title">
						<a class="label label-danger pull-right system position_delete" onclick="position_del();">批量删除</a>
	                    <a class="label label-success pull-right system position_add" onclick="forAddTable();">新增</a>
					</div>	
	...
	<script type="text/javascript">
	    ...
	    function findTable(){
	        ...
	        doAjaxPost('<%=request.getContextPath()%>/position.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
	    }
	    ...
	    function forAddTable(){
	    	var url = '<%=request.getContextPath()%>/position.action?act=forAddOrEdit&handle=add';
	    	defualtHref(url);
	    }
	    //准备修改
	    function forModifyTable(ui){
	        var url = '<%=request.getContextPath()%>/position.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
	        defualtHref(url);
	    }
	    //查看
	    function forLookTable(ui){
	        var url = '<%=request.getContextPath()%>/position.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
	        defualtHref(url);
	    }
	    ...
	    function selectAll(){
	    	if($("#selectAll").prop("checked")){
	    		$.each($("#position_list").find("input[name=chckBox]"),function(k,v){
	    ...
	    function del(id){
        	$.post('<%=request.getContextPath()%>/position.action?act=delete',{'id':id},function(data){
                var data=eval('(' + data + ')');
                if(data.msg=="true"){
              		layer.open({
               			skin: 'layui-layer-molv',
