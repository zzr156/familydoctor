                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;">服务内容编号</th>
                                <th style="text-align: center;">服务内容图标</th>
                                <th style="text-align: center;">服务内容名称</th>
...
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        ...
        if(iecs(data.serpkNum)){
            ui.find("td[pofield=pkNum]").text("-");
        }else{
...
<script type="text/javascript">
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/appPk.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/appPk.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        $.post('<%=request.getContextPath()%>/appPk.action?act=roleCmm',{'id':$(ui).data("vo").id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                var url = '<%=request.getContextPath()%>/appPk.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
                defualtHref(url);
            }else{
                showMsg(data.msg);
            }
    ...
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/appPk.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function delForm(ui){
        layer.confirm('确认删除数据?', {
            ...
            $.post('<%=request.getContextPath()%>/appPk.action?act=roleCmm',{'id':$(ui).data("vo").id,'method':'delete'},function(data){
                var data=eval('(' + data + ')');
                if(data.msg=="true"){
    ...
    function del(id){
        $.post('<%=request.getContextPath()%>/appPk.action?act=delete',{'id':id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功!");
                findTable();
    ...
    function openState(ui){
        $.post('<%=request.getContextPath()%>/appPk.action?act=openState',{'id':$(ui).data("vo").id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
    ...
    //标记
    function bj(ui){
        doAjaxPost('<%=request.getContextPath()%>/appPk.action?act=bjCmm',{id:$(ui).data("vo").id},function(data){
            if(data.msg=="true"){
