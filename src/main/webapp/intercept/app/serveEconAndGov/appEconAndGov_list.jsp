                ...
                <div class="ibox-content" id="form_qvo">
                    <form method="get" class="form-horizontal">
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control" pofield="econTitle"  id="econTitle">
                            </div>
                            <label class="col-sm-2 control-label">政府补贴类型：</label>
                            <div class="col-sm-4 ">
                                <input type="text" class="form-control" pofield="govTitle"  id="govTitle">
                        ...
                        <div class="text-center" style="border-top: 1px dashed #D7D7D7;padding-top: 10px;">
                            <a class="btn btn-primary system appEag_list" onclick="findTable();">查询</a>
                            <a class="btn btn-default system appEag_list" onclick="findReset();">重置</a>
                        </div>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <a class="label label-danger pull-right system appEag_delete" onclick="approle_del();">批量删除</a>
                    <a class="label label-success pull-right system appEag_add" onclick="forAddTable();">新增</a>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;width: 10%">经济类型名称</th>
                                <th style="text-align: center;">政府补贴类型名称</th>
                                <th style="text-align: center;">操作</th>
                            </tr>
                            </thead>
                            <tbody id="appEag_list">
...
<jsp:include page="appEconAndGov_change.jsp" flush="false"/>
<jsp:include page="/open/mould/app/serveEconAndGov/appEconAndGov_tmp.jsp" flush="false" />
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        ...
        ui.find("td[pofield=econTitle]").text(data.eagEconTitle);
        ui.find("td[pofield=govTitle]").text(data.eagGovTitle);
        if(data.eagOpenState=="1"){
...
//        findCmmInit();
    ...
    function findCmmInit(){
        doAjaxPost('<%=request.getContextPath()%>/appEag.action?act=findCmmInit',{},callDataToInit);
    }
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/appEag.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/appEag.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        $.post('<%=request.getContextPath()%>/appEag.action?act=roleCmm',{'id':$(ui).data("vo").id},function(data) {
            var data = eval('(' + data + ')');
            if (data.msg == "true") {
                var url = '<%=request.getContextPath()%>/appEag.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
                defualtHref(url);
            }else{
                showMsg(data.msg);
            }
    ...
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/appEag.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function selectAll(){
        if($("#selectAll").prop("checked")){
            $.each($("#appEag_list").find("input[name=chckBox]"),function(k,v){
    ...
    function delForm(ui){
        layer.confirm('确认删除数据?', {
            ...
            $.post('<%=request.getContextPath()%>/appEag.action?act=roleCmm',{'id':$(ui).data("vo").id,'method':'delete'},function(data){
                var data=eval('(' + data + ')');
                if(data.msg=="true"){
    ...
    function del(id){
        $.post('<%=request.getContextPath()%>/appEag.action?act=delete',{'id':id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功!");
                findTable();
    ...
    function openState(ui){
        $.post('<%=request.getContextPath()%>/appEag.action?act=openState',{'id':$(ui).data("vo").id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
