        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <a class="label label-danger pull-right system appdev_delete" onclick="hosp_del();">批量删除</a>
                    <a class="label label-success pull-right system appdev_add" onclick="forAddTable();">新增</a>
                </div>
...
<jsp:include page="/open/mould/app/device/device_tmp.jsp" flush="false" />
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        var ui = $($("#" + id + "").html());
        ui.data("vo", data);
        ui.find("td[pofield=devId] input[name=chckBox]").val(data.id);
        ui.find("td[pofield=num]").text(idx+1);
        ui.find("td[pofield=devName]").text(data.devName);
        ui.find("td[pofield=devFactoryModel]").text(data.devFactoryModel);
        ui.find("td[pofield=devType]").text(data.devTypeName);
        ui.find("td[pofield=devImage]").html('<img width="200" height="100" src="<%=request.getContextPath()%>/image.action?act=getUrlImage&url='+data.devImageUrl+'"/>');
        ui.find("td[pofield=devCreateName]").text(data.devCreateName);
        ui.find("td[pofield=devCreateTime]").text(data.strDevCreateTime);
        $("#device_list").append(ui);
        ui.fadeIn();
    }
...
<script type="text/javascript">
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/appdev.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/appdev.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        var url = '<%=request.getContextPath()%>/appdev.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    //查看
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/appdev.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function selectAll(){
        if($("#selectAll").prop("checked")){
            $.each($("#device_list").find("input[name=chckBox]"),function(k,v){
    ...
    function del(Ids){
        $.post('<%=request.getContextPath()%>/appdev.action?act=delete',{'id':Ids},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功！");
                findTable();
