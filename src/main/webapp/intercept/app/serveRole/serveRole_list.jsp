                ...
                <div class="ibox-content" id="form_qvo">
                    <form method="get" class="form-horizontal">
                         <div class="form-group">
                             ...
                             <div class="col-sm-4 ">
                                 <select class="form-control" pofield="serveType" id="serveType">
                         ...
                         <div class="text-center" style="border-top: 1px dashed #D7D7D7;padding-top: 10px;">
                             <a class="btn btn-primary system appserverole_list" onclick="findTable();">查询</a>
                             <a class="btn btn-default system appserverole_list" onclick="findReset();">重置</a>
                         </div>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <a class="label label-danger pull-right system appserverole_delete" onclick="module_del();">批量删除</a>
                    <a class="label label-success pull-right system appserverole_add" onclick="forAddTable();">新增</a>
                </div>
...
<jsp:include page="/open/mould/app/serveRole/serve_tmp.jsp" flush="false" />
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        ...
        ui.find("td[pofield=serveName]").text(data.serveName);
        ui.find("td[pofield=strServeState]").text(data.strServeState);
        ui.find("td[pofield=strServeAreaCode]").text(data.strServeAreaCode);
        $("#serverole_list").append(ui);
        ui.fadeIn();
    }
...
<script type="text/javascript">
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/appserverole.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/appserverole.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        var url = '<%=request.getContextPath()%>/appserverole.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    //查看
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/appserverole.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function forRoleMenuTable(ui){
        ...
        var url = '<%=request.getContextPath()%>/appserverole.action?act=forAddOrEditRole&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function module_del(){
        var Ids="";
        $.each($("#serverole_list").find("input[name=chckBox]"),function(k,v){
    ...
    function del(Ids){
        $.post('<%=request.getContextPath()%>/appserverole.action?act=delete',{'id':Ids},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功！");
                findTable();
