                ...
                <div class="ibox-content" id="form_qvo">
                    <form method="get" class="form-horizontal">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">经济类型名称：</label>
                            <div class="col-sm-4">
                        ...
                        <div class="text-center" style="border-top: 1px dashed #D7D7D7;padding-top: 10px;">
                            <a class="btn btn-primary system appEcon_list" onclick="findTable();">查询</a>
                            <a class="btn btn-default system appEcon_list" onclick="findReset();">重置</a>
                        </div>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <a class="label label-danger pull-right system appEcon_delete" onclick="approle_del();">批量删除</a>
                    <a class="label label-success pull-right system appEcon_add" onclick="forAddTable();">新增</a>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;">经济类型值</th>
                                <th style="text-align: center;">经济类型名称</th>
                                <th style="text-align: center;">操作</th>
                            </tr>
                            </thead>
                            <tbody id="appEcon_list">
...
<jsp:include page="appSerEcon_change.jsp" flush="false"/>
<jsp:include page="/open/mould/app/serveEcon/appServeEcon_tmp.jsp" flush="false" />
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        var ui = $($("#" + id + "").html());
        ui.data("vo", data);
        ui.find("td[pofield=eId] input[name=chckBox]").val(data.id);
        ui.find("td[pofield=num]").text(idx+1);
        ui.find("td[pofield=value]").text(data.econValue);
        ui.find("td[pofield=title]").text(data.econTitle);
        if(data.econOpenState=="1"){
...
<script type="text/javascript">
    ...
    function findCmmInit(){
        doAjaxPost('<%=request.getContextPath()%>/appEcon.action?act=findCmmList',{},callDataToInit);
    }
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/appEcon.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/appEcon.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        $.post('<%=request.getContextPath()%>/appEcon.action?act=roleCmm',{'id':$(ui).data("vo").id},function(data) {
            var data = eval('(' + data + ')');
            if (data.msg == "true") {
                var url = '<%=request.getContextPath()%>/appEcon.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
                defualtHref(url);
            }else{
                showMsg(data.msg);
            }
    ...
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/appEcon.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function selectAll(){
        if($("#selectAll").prop("checked")){
            $.each($("#appEcon_list").find("input[name=chckBox]"),function(k,v){
    ...
    function delForm(ui){
        layer.confirm('确认删除数据?', {
            ...
            $.post('<%=request.getContextPath()%>/appEcon.action?act=roleCmm',{'id':$(ui).data("vo").id,'method':'delete'},function(data){
                var data=eval('(' + data + ')');
                if(data.msg=="true"){
    ...
    function del(id){
        $.post('<%=request.getContextPath()%>/appEcon.action?act=delete',{'id':id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功!");
                findTable();
    ...
    function openState(ui){
        $.post('<%=request.getContextPath()%>/appEcon.action?act=openState',{'id':$(ui).data("vo").id},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
    ...
    //标记
    function bj(ui){
        doAjaxPost('<%=request.getContextPath()%>/appEcon.action?act=bjCmm',{id:$(ui).data("vo").id},function(data){
            if(data.msg=="true"){
                $.each($("#appEcon_list").find("button[pofield=bj]"),function(k,v){
