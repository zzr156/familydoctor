                ...
                <div class="ibox-content" id="form_qvo">
                    <form method="get" class="form-horizontal">
                         ...
                         <div class="text-center" style="border-top: 1px dashed #D7D7D7;padding-top: 10px;">
                             <a class="btn btn-primary system videoAction_list" onclick="findTable();">查询</a>
                             <a class="btn btn-default system videoAction_list" onclick="findReset();">重置</a>
                         </div>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <a class="label label-danger pull-right system videoAction_delete" onclick="menu_del();">批量删除</a>
                    <a class="label label-success pull-right system videoAction_add" onclick="forAddTable();">新增</a>
                </div>
...
<jsp:include page="/open/mould/app/video/video_tmp.jsp" flush="false" />
<script type="text/javascript">
    function uiFromTmp(id, data,idx) {
        ...
        ui.find("td[pofield=videoName]").text(data.videoName);
        ui.find("td[pofield=videoContent]").text(data.videoContent);
        ui.find("td[pofield=videoAddress]").text(data.videoAddress)
        ui.find("td[pofield=strVideoState]").text(data.strVideoState);
...
<script type="text/javascript">
    ...
    function findTable(){
        ...
        doAjaxPost('<%=request.getContextPath()%>/videoAction.action?act=list',{qvoJson:JSON.stringify(qvo)},callDataToUi);
    }
    ...
    function forAddTable(){
        var url = '<%=request.getContextPath()%>/videoAction.action?act=forAddOrEdit&handle=add';
        defualtHref(url);
    }
    //准备修改
    function forModifyTable(ui){
        var url = '<%=request.getContextPath()%>/videoAction.action?act=forAddOrEdit&handle=modify&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    //查看
    function forLookTable(ui){
        var url = '<%=request.getContextPath()%>/videoAction.action?act=forAddOrEdit&handle=look&vo.id='+$(ui).data("vo").id;
        defualtHref(url);
    }
    ...
    function del(Ids){
        $.post('<%=request.getContextPath()%>/videoAction.action?act=delete',{'id':Ids},function(data){
            var data=eval('(' + data + ')');
            if(data.msg=="true"){
                showMsg("删除成功！");
                findTable();
