...
var Etypes=getQueryString("Etypes");
var orgValue=getQueryString("orgValue");
var orgValues=getQueryString("orgValues");
var CodeNmae='';
var upWebAreaCode = '';
var nowtime = '';
var teamKey = [];
...
*/
    ...
    doAjaxPost('signAction.action?act=appManageIndexCountNew',{strJson:JSON.stringify(vo)},function (data) {
        ...
        if(data.map.signTotalState.total!=null){
            ...
            upWebAreaCode = data.map.signTotal.upWebAreaCode;
            $("#OrgName").html(CodeNmae+"家庭医生签约服务情况统计");
      ...
      var  option3 = {
        ...
        Chart_qyzt.on('click', function (params) {
            if(orglist[params.dataIndex] != undefined){
                if(orglist[params.dataIndex].length>10){
                    window.location.href="echartses.jsp?Etype=false&orgValue="+orglist[params.dataIndex]+"&goto=2&orgValues="+orgValue+"&Etypes="+Etype+"&CodeNmae="+encodeURI(params.name);
                }else {
                    window.location.href="echartses.jsp?Etype=true&orgValue="+orglist[params.dataIndex]+"&goto=2&orgValues="+orgValue+"&Etypes="+Etype+"&CodeNmae="+encodeURI(params.name);
                }
            }else {
                openPoorSign(params);
            }
        });
        if(HospLevel==data.map.signTotal.hospLevel){
            $("#comeback").remove();
        }
        ...
        if(HospAreaCode.substr(0,4)=="3503"){
            if(data.map.signTotal.hospLevel.indexOf("4")>=0){
                $("#ptLevel").remove();
            }else{
                onptTable(data.map.signTotal.hospLevel);
            }
            gxrklist=[];
            rkzslist=[];
...
function signsx(){
    if(goto=="1"){
        window.location.href="echartses.jsp?Etype="+Etype+"&orgValue="+orgValue+"&goto="+goto+"&CodeNmae="+CodeNmae;
    }else if(goto=="2"){
        window.location.href="echartses.jsp?Etype="+Etype+"&orgValue="+orgValue+"&goto=2&orgValues="+orgValues+"&Etypes="+Etypes+"&CodeNmae="+CodeNmae;
    }
...
function signgoto(){
    ...
    if(upWebAreaCode == '140000000000'){
        window.location.href="echarts.jsp?&CodeNmae="+encodeURI(CodeNmae);
    }else{
        window.location.href="echartses.jsp?Etype="+Etypes+"&orgValue="+upWebAreaCode+"&goto=1&CodeNmae="+encodeURI(CodeNmae);
    }
}
