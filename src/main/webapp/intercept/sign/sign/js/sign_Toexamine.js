...
function ptInitTable() {
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{fixed: 'right', width:340, align:'center', toolbar: '#barRole'}
        ]]
        ,id: 'sign'
        ,url: 'signAction.action'
        ,where: {act: 'findSignToexamine',strJson:JSON.stringify(qvo)}
        ,method: 'post'
        ,skin: 'row' //表格风格
        // ,size: 'sm'
        ,even: true
...
function forTeamModify(ui) {
    ...
    var url = "sign_modify.jsp?id="+ui.id+"&toexamine=1";
...
function refuse(ui) {
    myOpen("拒签信息","sign_refuse.jsp?signId="+ui.id,findList);
}