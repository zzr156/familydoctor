...
$(function(){
    ...
    if(jdSourceType == "0"){
       //医保查询隐藏低保和特困
        ...
        /*查询当前医生权限判断是否有权修改身份证号（有市和区权限才能修改）*/
        doAjaxPost('appdr.action?act=jsonByOne',{id:drid},function(data){
            if(data != null){
                if(data.drRole != null){
                    if(data.drRole.indexOf("2")!=-1 || data.drRole.indexOf("3")!=-1){
                        if(HospAreaCode != null && HospAreaCode != "" && HospAreaCode.substring(0,4) == "3501"){
    ...
    if(id!=null && id!=""){
        ...
        doAjaxPost('archivingAction.action?act=archivingLook',{strJson:JSON.stringify(vo)},function(data){
            if(data.vo!=null) {
                ...
                if(data.vo.notSignReason==20 && data.vo.signState == "未签约"){
                    // $("#yyid").show();
    ...
    if($("#addrRural").val()!=null && $("#addrRural").val()!=""){
        ...
        doAjaxPost('archivingAction.action?act=findJnameList',{strJson: JSON.stringify(vo)},function(data){
            ...
            if(data!=null && data.rows != null){
                ...
                var osel=document.getElementById("addrVillage"); //得到select的ID
                var opts=osel.getElementsByTagName("option");//得到数组option
                if(opts.length>0){
                    for(var i=0;i<opts.length;i++){
                        if(opts[i].text==$("#addrVillageName").val()){
                            opts[i].selected=true
                        }
                    }
                }
              /*  if($("#addrVillageName").val()){
                    $("#addrVillage").text($("#addrVillageName").val());
                }*/
            }
        },function(data){
            layer.msg("级联初始化异常，请联系管理员！");
