        ...
        if($("#patientStreet").val()!=""){
            table.render({
                ...
                ,cols: [[
                    ...
                    ,{field: 'hospName', title: '签约机构',width:100}
                    ,{field: 'addrHospName', title: '归管机构',width:100}
...
function exportCurrent(num) {
    ...
    window.open("archivingAction.action?act=findArchivingXxWebExcel&typeNum="+num+"&strJson="+JSON.stringify(qvo)+"&numberCount="+numberCount);

}
function forTeamModify(ui) {
    var id=ui.id;
    var url = "archiving_modify.jsp?id="+ui.id;
    defualtHref(url);
    // doAjaxPost('archivingAction.action?act=findDeleteState', {id,id}, function (data) {
    //     if(data.code=="900"){
    //         layer.msg("该数据为删除状态，不可修改");
    //     }else{
    //         var url = "archiving_modify.jsp?id="+ui.id;
    //         defualtHref(url);
    //     }
