...
var healthType = getQueryString("healthType");
...
function initData(){
    ...
    doAjaxPost('signAction.action?act=getAchievementsList', {strJson: JSON.stringify(vo)}, function (data) {
        if(data.vo != null){
            ...
            if(object.gxysfRecordDTOList.length == "0" && object.tnbsfRecordDTOList.length == "0" && object.jkjtRecordDTOList.length == "0" && object.jsbsfRecordDTOList.length == "0" && object.fjhsfFirstRecordDTOList.length == "0" && object.fjhsfRecordDTOList.length == "0"){
                $("#serverLog").text("无");
            }
            initDoctor();
        }
...
function addReport(){
    if(validator("form_qvo")) {
        ...
        doAjaxPost('signAction.action?act=findHealthReport', {strJson: JSON.stringify(vo)}, function (data) {
            if(data.vo == null){
                doAjaxPost('signAction.action?act=addHealthReport', {strJson: JSON.stringify(vo)}, function (data) {
                    if(data.code == "800"){
                        layer.msg("保存报告成功！");
                    }else {
                        layer.msg(data.msg);
