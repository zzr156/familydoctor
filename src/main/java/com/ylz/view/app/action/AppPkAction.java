...
@Action(
        value="appPk",
        results={
                @Result(name="list", location="/intercept/app/servePk/appSerPk_list.jsp"),
                @Result(name="edit", location="/intercept/app/servePk/appSerPk_edit.jsp"),
...
public class AppPkAction extends CommonAction {
        ...
        try {
            ...
            if (vo != null) {
               ...
               if(table!=null){
                   ...
                   table.setSerpkTime(vo.getSerpkTime());
                   table.setSerpkItem(vo.getSerpkItem());
                   //session出现实体重复时的处理
    ...
    public String findCmmInit(){
        try {
            ...
            //是否
            List<CdCode> openState = this.getSysDao().getCodeDao().findGroupList(CodeGroupConstrats.GROUP_PCSTATE, CommonEnable.QIYONG.getValue());
            //图标
            ...
            String code = sysDao.getAppServePackageDao().findCode();
            String areaCode = "";
            CdUser user = this.getSessionUser();
            if(user != null) {
                ...
                if(user!=null){
                    if(!"admin".equals(user.getAccount())  && !"smadmin".equals(user.getAccount())&&
                        ...
                        if(!user.getHospId().equals(pk.getSerpkDeptId())){
                            ...
                            return "json";
                        }else if("delete".equals(method)&&sysDao.getAppServePackageDao().isReferencedByGroup(pk)){
                            this.newMsgJson("有[服务组合]正在引用该[服务内容],无法删除!");
                            return "json";
                        }
                    }
                }
