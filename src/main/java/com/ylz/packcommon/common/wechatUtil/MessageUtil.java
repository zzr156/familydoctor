...
import com.ylz.packcommon.common.wechat.message.response.*;
...
public class MessageUtil {
	...
	public static Map<String, String> parseXml(HttpServletRequest request) throws Exception {
		...
		try {
			...
			log.error("解析微信发来的请求（request转XML）异常"+e);
			e.printStackTrace();
		}
	 ...
	 * @return xml
	 */
	public static String encrptToXml(Encrpt encrpt) {
		xstream.alias("xml", encrpt.getClass());
		return xstream.toXML(encrpt);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static Map<String, String> StrXml(String xml) throws Exception {
		...
		try {
		    ...
		    Document document = sr.read(re);
			// 得到xml根元素
			Element root = document.getRootElement();
			// 得到根元素的所有子节点
			List<Element> elementList = root.elements();
			// 遍历所有子节点
			if((elementList==null)||(elementList!=null&&elementList.size()<1)){
				return null;
			}
			for (Element e : elementList){
				map.put(e.getName(), e.getText());
